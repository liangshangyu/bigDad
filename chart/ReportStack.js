...
export default class AnaComparisonStack extends BaseChart {
  ...
  draw (option) {
    ...
    axisTxts.style('fill', 'white').style('font-size', '12px').style('display', 'none')
    ...
    graph.selectAll('g').data(data).enter().append('g')
      .classed('stack', true).each(function (d, i) {
        ...
        .attr('fill', profileLineColor).transition().duration(200).delay(i * 50)
        .attr('height', yScale(d.value) + 20)
        .attr('y', totalH - yScale(d.value) - 10)
        d3.select(this).append('rect')
